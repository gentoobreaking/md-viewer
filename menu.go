...
func RegisterOpenFileCallback(callback func(string)) {
	...
	C.FlushPendingOpenFile()
}

// EnsureNSAppDelegateInstalled installs NSApplication.sharedApplication and sets the
// app delegate before webview init, so Finder open-file events are not lost.
func EnsureNSAppDelegateInstalled() {
	C.EnsureNSAppDelegateInstalled()
}
