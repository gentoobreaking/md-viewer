...
func prepareHTML(html string) string {
	...
	// Pass resolved mermaid theme to head for sync initialization
	mermaidTheme := theme
	if mermaidTheme == "dark" || mermaidTheme == "solarized" || mermaidTheme == "nord" {
		mermaidTheme = "dark"
	} else {
		mermaidTheme = "default"
	}
	html = strings.Replace(html, "</head>", `<script>window.__mermaidTheme="`+mermaidTheme+`";</script></head>`, 1)
	return html
}
...
func renderMD(md string) string {
	...
	// Use placeholders to avoid display math content interfering with inline processing

	// Step 1: Escape escaped dollar signs (so we don't process them as math)
	escapedPlaceholder := "\x00ESC\x00"
	html = strings.ReplaceAll(html, `\$`, escapedPlaceholder)

	// Step 2: Find and replace display math $$...$$ with unique placeholders
	// Use (?s) so . matches newlines
	displayRegex := regexp.MustCompile(`(?s)\$\$(.+?)\$\$`)
	displayMathCache := make(map[string]string)
	displayCounter := 0
	html = displayRegex.ReplaceAllStringFunc(html, func(match string) string {
		content := displayRegex.FindStringSubmatch(match)[1]
		placeholder := fmt.Sprintf("{{MATH_DISPLAY_%d}}", displayCounter)
		displayMathCache[placeholder] = content
		displayCounter++
		return placeholder
	})

	// Step 3: Now process inline math $...$ (simple regex, no $$ interference)
	inlineRegex := regexp.MustCompile(`\$(.+?)\$`)
	html = inlineRegex.ReplaceAllString(html, "<span class=\"katex-inline\">$$1$$</span>")

	// Step 4: Restore display math with KaTeX div wrappers
	for placeholder, content := range displayMathCache {
		html = strings.ReplaceAll(html, placeholder, "<div class=\"katex\">$$"+content+"$$</div>")
	}

	// Step 5: Restore escaped dollar signs
	html = strings.ReplaceAll(html, escapedPlaceholder, `\$`)
